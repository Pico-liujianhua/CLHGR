...
class CLDataCollator:
    max_len: int
    args: Namespace
    len_sampling_bound = [0.3, 0.7] #随机采样长度比例
    dense_sampling_bound = [0.4, 0.6] #在随机采样后的密集与稀疏数据分布比例
    ...
    def __call__(self, batch):
        #自定义DataLoader对每个batch的处理方式，即怎么把一组数据组合成batch
        batch_size = len(batch)
        D = batch[0][0].size(1)
        print('CLDataCollator batch_size '+str(batch_size)) #8，参数决定
        print('CLDataCollator D '+str(D)) #1
    ...
    def _per_seq_sampling(self, instance):
        ...
        时间序列数据的采样与掩码，主要目的是将输入数据拆分成两部分 (v1, t1, m1, m1_old) 和 (v2, t2, m2, m2_old)，
        适用于时间序列对比学习（contrastive learning）。
        ...
        if self.pretrain_tasks == 'full2':
            ...
            m1 = self._seg_masking(mask=m1, timestamps=t1) #进行掩码
            # a = m1[ : , : D]
            # b = m1[ : , D : ]
            # c = a + b
    ...
    def _time_sensitive_cl(self, timestamps):
        ...
        #把times，avg_interval_times和times下标组成三元组
        pairs = [(idx, time, avg_interval_time) for idx, (time, avg_interval_time) in
                 enumerate(zip(times, avg_interval_times))]
        # print(pairs)
        #按照avg_interval_time从小到大排序，排序结束后密集的时间戳在前面，稀疏的时间戳在后面
        pairs.sort(key=lambda pairs: pairs[2]) #按时间间隔从小到大排序（最密集的时间步在前）
        indices = [idx for idx, time, avg_interval_time in pairs] #获取排序后的时间戳索引
        # print(pairs)

        # length of the anchor/positive sample
        #np.random.uniform(a, b, size) 生成 size 个 均匀分布 的随机数，[0]取第一个元素
        length = int(np.random.uniform(self.len_sampling_bound[0], self.len_sampling_bound[1], 1)[0] * times.shape[0])#确定采样长度
        length = max(length, 1)
        # print(length)
    ...
    def _seg_masking(self, mask=None, timestamps=None):
        ...
        mask (T, D)：原始掩码，已应用时间敏感屏蔽，即需要掩码的位置已设置为0
        interp_mask (T, D)：插值掩码，标记需要插值的位置
        '''
        #D可以看做特征维度，即一个时间点采集的信息个数，单通道传感器就是1
        D = mask.size(1)
        interp_mask = torch.zeros_like(mask)
        #print('_seg_masking Dimension: ' + str(D)) #UCR数据是1，这是正确的,EMG也是1
    ...
    def _time_sensitive_sampling(self, mask, timestamps):
        '''
        基于时间信息的动态采样，选择某些时间点进行“掩码”操作，主要目的是 模仿真实数据的缺失情况，从而提高模型对不规则时间序列的适应性
        '''

        # segment_num = 3
        # mask_ratio_per_seg = 0.15

        timestamps = timestamps.reshape(timestamps.shape[0]) #把timestamps从二维(T,1)变为一维张量
        # sampled_times = timestamps[mask].tolist() # times at which this feature was sampled,挑出有效的采样
        ...
        if len(available_samples_to_sample) > 0:
            print('available_samples_to_sample: ' + str(len(available_samples_to_sample)))
        ...
        masking_times = []
        for i in range(self.args.segment_num): #segment_num决定总共屏蔽多少个时间段
            ...
            #从chosen_time向前遍历可采样的点，若满足要求就加入采样点
            ...
            #反向遍历，剔除与 chosen_time 过近的时间点，确保后续选取的新 chosen_time 远离当前已屏蔽区域。
            for time in sampled_times[: idx][::-1]: 
