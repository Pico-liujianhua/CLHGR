...
def generate_mask(data, p = 0.5, remain = 0):
    '''
    p: 遮挡的概率，默认 50% 的数据会被掩盖
    remain: 指定数据中必须保留不被遮挡的最后 remain 个时间步
    return: 和data形状一样的mask 由0和1组成
    '''
    B, T, C = data.shape           #样本数x时间步数x通道数
    mask = np.empty_like(data)

    for b in range(B):
        ts = data[b, :, 0]
        et_num = ts[~np.isnan(ts)].size - remain  #计算有效时间步数(去除NAN数据后)
        total, num = et_num * C, round(et_num * C * p) #所有通道上的总数据点数； 需要被遮挡的元素

        while True:
            i_mask = np.zeros(total)
            i_mask[random.sample(range(total), num)] = 1  #在total中选取 num 个索引置 1（即遮挡的部分）
            i_mask = i_mask.reshape(et_num, C)
            if 1 not in i_mask.sum(axis = 0) and 0 not in i_mask.sum(axis = 0): #axis=0按列求和
                break  #确保每个通道都有 0 和 1，避免某个通道数据全部被遮挡或全部保留
            break

        i_mask = np.concatenate((i_mask, np.ones((remain, C))), axis = 0) #确保最后remain个数据不会被遮挡
