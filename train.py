...
if __name__ == '__main__':
    ...
    parser.add_argument('--segment_num', type=int, default=3,
                        help='number of time interval segment to mask, default: 3 time intervals') #决定总共屏蔽多少个时间段
    ...
    elif args.loader == 'EMG':
        task_type = 'classification'
        train_data, train_labels, test_data, test_labels =load_emg.load_EMG_from_csv(args.dataset, load_tp = args.load_tp)
    ...
    print('done')
    print(train_data['x'].shape) #实际训练数据的形状 UCR(100, 1460, 2) 样本数x时间步数x通道数，添加了一个时间通道
