...
def load_UCR(dataset, load_tp: bool = True):
    ...
    '''
    在数据中追加一个时间戳信息,给每个样本的每个数据点赋值一个时间戳,值为0~1的归一化,目的是告诉网络信息的先后顺序
    '''
    if load_tp:
        tp = np.linspace(0, 1, train.shape[1], endpoint=True).reshape(1, -1, 1) #np.linspace 生成从 0 到 1 的等间隔序列，长度等于时间步数 train.shape[1]（即序列长度）
