...
class TimesURL:
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False, is_scheduler=True, temp=1.0):
        ...
        '''
        train_data, mask = train_data['x'], train_data['mask'] #(100, 1460, 2)

        assert train_data.ndim == 3 #确保是三个维度的数据

        if n_iters is None and n_epochs is None:
            n_iters = 200 if train_data.size <= 100000 else 600  # default param for n_iters 动态选择优化器更新次数
        ...
        if self.max_train_length is not None:  
            ...
            if sections >= 2:#如果数据太长，进行切分，UCR不用切分
